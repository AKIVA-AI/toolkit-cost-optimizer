"""
Control-plane adapter for toolkit-cost-optimizer.
...
Toolkit-cost-optimizer is an Archetype 9 CLI tool (TK-01).  The control-plane
contract here covers:
  - Config hierarchy (global defaults → toolkit config → CLI overrides)
  - CLI command → ToolSpec mapping for all 4 subcommands
