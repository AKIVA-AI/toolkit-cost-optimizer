                    ...
                    cost_data.append(
                        {
                            ...
                            "resource_type": (
                                keys[1].split("/")[-1] if len(keys) > 1 and "/" in keys[1] else None
                            ),
                            "tag_environment": (
                                keys[2].split("$")[-1] if len(keys) > 2 and "$" in keys[2] else None
                            ),
                            "cost_amount": Decimal(
                ...
                for row in result:
                    ...
                    if utilization_score < 0.3:
                        recommendations.append(
                            "Consider right-sizing or terminating underutilized resource",
                        )
                    if utilization_score > 0.9:
                        recommendations.append("Resource may be overutilized - consider scaling up")
                    if efficiency_score > 0.7:
                        recommendations.append(
                            "High optimization potential - review resource configuration",
                        )
        ...
        async with get_db_session() as session:
            try:
                ...
                daily_costs = await session.execute(
                    ...
                    .group_by(CostData.usage_start_date)
                    .order_by(CostData.usage_start_date),
                )
            ...
            try:
                ...
                for row in weekly_costs:
                    ...
                    if len(costs) > 1:
                        ...
                        cost_change_percentage = (
                            (cost_change / costs[-2]) * 100 if costs[-2] > 0 else 0
                        )
                        ...
                        if len(costs) >= 3:
                            recent_changes = [costs[j] - costs[j - 1] for j in range(-3, 0)]
                            avg_change = abs(
                                sum(recent_changes) / len(recent_changes),
                            )
                            trend_strength = (
                                min(
                                    avg_change / costs[-1] * 100,
                                    100,
                                )
                                / 100
                            )
