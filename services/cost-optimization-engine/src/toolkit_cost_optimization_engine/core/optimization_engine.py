                ...
                for row in result:
                    ...
                    if (
                        ...
                        if current_cost > 0:
                            ...
                            util_ratio = (
                                min(avg_util / (threshold * 100), 1.0) if threshold > 0 else 0
                            )
                            savings_percentage = 0.50 - (
                                util_ratio * 0.20
                            )  # 0.50 at 0% util, 0.30 at threshold
                            monthly_savings = current_cost * Decimal(
                                str(round(savings_percentage, 4)),
                            )
                    ...
                    if self._has_business_hours_pattern(hourly_pattern):
                        ...
                        if current_cost > 0:
                            ...
                            recommendation = RecommendationData(
                                ...
                                priority=(
                                    Priority.MEDIUM if monthly_savings > 100 else Priority.LOW
                                ),
                                implementation_steps=[
                                ...
                                rollback_plan=(
                                    "Disable scheduling and run resource 24/7 if issues occur"
                                ),
                                analysis_data={
    ...
    async def analyze(self, context: OptimizationContext) -> list[RecommendationData]:
        ...
        async with get_db_session() as session:
            try:
                ...
                idle_days = optimization_settings.IDLE_RESOURCE_TIMEOUT_HOURS // 24
            ...
            try:
                ...
                for row in result:
                    ...
                    if current_cost > 100:  # Only recommend for significant costs
                        ...
                        savings_percentage = (
                            0.30 + (stability_factor * 0.15) + (usage_factor * 0.15)
                        )
        ...
        try:
            ...
            for rule in self.rules:
                try:
                    ...
                    logger.info(
                        f"Generated {len(recommendations)} recommendations from {rule.name}",
                    )
                except Exception as e:
